...
function addSection(pdf: jsPDF, title: string, text: string, currentY: number): number {
  ...
  pdf.setDrawColor(226, 232, 240); // border line
  ...
  currentY = checkPageBreak(pdf, currentY, textHeight);
...
/**
 * Formats and adds a monospace code block to the PDF report
 */
function addCodeBlock(pdf: jsPDF, title: string, code: string, currentY: number): number {
  ...
  // Monospace block background
  pdf.setFont("courier", "normal");
  pdf.setFontSize(8);
  pdf.setTextColor(15, 118, 110); // teal-700
  
  const splitCode = pdf.splitTextToSize(code, pageWidth - margin * 2);
  const blockHeight = splitCode.length * 4.2 + 6;
  
  currentY = checkPageBreak(pdf, currentY, blockHeight);
  
  pdf.setFillColor(248, 250, 252); // slate-50
  pdf.rect(margin, currentY - 2, pageWidth - margin * 2, blockHeight, "F");
  
  pdf.text(splitCode, margin + 4, currentY + 3);
  
  return currentY + blockHeight + 10;
}

/**
 * Captures the circuit diagram element as a PNG and compiles a PDF report
 */
