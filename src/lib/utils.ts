...
export function exportMarkdownToPDF(title: string, mdText: string) {
      ...
      if (inTable) {
        ...
        for (let row of tableRows) {
          ...
          if (isFirstRow) {
            ...
            tableHtml += `<tr class="border-b border-gray-200 hover:bg-gray-50">${cells.map((c) => `<td class="px-4 py-2 border border-gray-200 text-sm">${c}</td>`).join("")}</tr>`;
          }
        }
  ...
  bodyContent = bodyContent
    .replace(/^(\s*)\* (.*$)/gim, '<li class="ml-6 list-disc my-1">$2</li>')
    .replace(/^(\s*)- (.*$)/gim, '<li class="ml-6 list-disc my-1">$2</li>')
    .replace(/^(\s*)\d+\. (.*$)/gim, '<li class="ml-6 list-decimal my-1">$2</li>');

  bodyContent = bodyContent
    .replace(/^# (.*$)/gim, '<h1 class="text-3xl font-bold mt-8 mb-4 text-blue-900 border-b-2 border-blue-100 pb-2">$1</h1>')
    .replace(/^## (.*$)/gim, '<h2 class="text-2xl font-semibold mt-6 mb-3 text-blue-800 border-b border-gray-200 pb-1">$1</h2>')
    .replace(/^### (.*$)/gim, '<h3 class="text-xl font-medium mt-5 mb-2 text-gray-800">$1</h3>');
